...
export default {
  ...
  // 定义表单内的数据(初始为空)
  data () {
    // 里面有三个参数rule(验证规则), value(要验证的内容), callback(回调函数)
    let validator = function (rule, value, callBack) {
      // 如果勾选了(结果为true)则通过,执行回调函数
      if (value === true) {
        callBack() // 什么都不写为通过
      } else {
        // 为false,抛出错误信息
    ...
    return {
      ...
      rules: {
        mobile: [
          // 验证表单是否填写内容,required为true表示必须有内容,message表示没有输入内容时的提示信息,required只校验null '' undefined
